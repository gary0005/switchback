...
**certbot belongs on this machine, not on the servers.** Certificates are issued here and only the resulting files are shipped out — see [roles/acme/README.md](../roles/acme/README.md).
...
The first domain is the node's primary — the name chains and subscription links address it by, so it must reach that node and no other. The rest are aliases and go into the same certificate. The apex can sit on several nodes at once: when a certificate is issued for it, the challenge file is copied to all of them.
...
**No DNS API key is needed anywhere.** Certificates come over http-01, and the challenge is copied to every node that answers for the name using the ssh access you already have. There is no zone credential to create, store or rotate.

## 5. DNS

Records are maintained by hand, wherever your domain's nameservers point. There is no Terraform: they change when you add a node or move a provider, which is rare enough that automating it costs more than it saves. `--tags verify` is what notices if they drift.

One A record per name in `vpn_domains`. Per-node names point at one node each; **the apex may point at several at once** — visitors round-robin between them, and every one of those nodes serves the same site.
...
That the apex spans three nodes is exactly why the challenge is distributed: Let's Encrypt reaches one of the three, and the file has to be on whichever it picks.

Records must resolve **before** the first run — a certificate cannot be issued for a name that does not point anywhere:
