...
4. **Nothing reaches xray from a network that is inspected.** A chain leaving a node in the `domestic` group disguises its second hop as ordinary HTTPS to the far node's angie — that leg crosses the filtering border and direct xray flows get banned fast. A chain between two nodes abroad goes straight to xray's Reality port, which is a TLS termination cheaper and safe because nothing inspects it. The port is opened to peer addresses alone, and `verify` asserts both halves: xray holds that port and no other, and the firewall restricts it by source.
...
- Reality's `dest` must be reachable **from the node running the inbound** — it proxies the handshake there, so an unreachable dest makes every connection hang rather than fail. `www.microsoft.com` times out from Riga; `www.cloudflare.com` works from both. Check with curl from each node before changing it.
- `ss -ltunp` lists outbound UDP sockets as UNCONN, indistinguishable from listeners. Filter the kernel's ephemeral range (`/proc/sys/net/ipv4/ip_local_port_range`) or xray's own outgoing connections read as ports it opened.
